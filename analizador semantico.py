...
def evaluar_expresion(expr):
    tokens = expr.split('+') #Se divicen los digitos cada vez que hay un mas.
    final_tipo = None
    for token in tokens:
        token = token.strip()
        if re.match(r'^\d+$', token): #para identificar un entero
            this_tipo = 'int'
        elif re.match(r'^\d+\.\d+$', token):#si lleva punto el numero es float
            this_tipo = 'float'
...
def analizador(code_lines):
    for line in code_lines:
        line = line.strip()
        if not line or line.startswith("//"): #Omitir codigo comementado.
